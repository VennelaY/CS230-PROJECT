...
class CACHE : public MEMORY {
  public:
    ...
    void add_mshr(PACKET *packet),
         ...
         rnd_update(uint32_t set, uint32_t way),
    ...
    void back_invalidate1(uint64_t address);
    void invalidate(uint32_t cpu, uint64_t address);
...
lfu_victim(uint32_t cpu, uint64_t instr_id, uint32_t set, const BLOCK *current_set, uint64_t ip, uint64_t full_addr, uint32_t type),
             ...
             rnd_victim(uint32_t cpu, uint64_t instr_id, uint32_t set, const BLOCK *current_set, uint64_t ip, uint64_t full_addr, uint32_t type);
             };
